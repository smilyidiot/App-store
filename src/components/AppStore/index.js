...
class Abcd extends Component {
  ...
  render() {
    const {appCategory, searchInput} = this.state
    ...
    const filteredApps = searchResults.filter(
      eachApp => eachApp.category === appCategory,
    )
      ...
      <div className="bg-cont">
        ...
        <ul className="tabItem">
          {tabsList.map(element => (
            <TabItem
              tabItem={element}
              displayApp={this.showTabId}
              isActive={element.tabId === appCategory}
              key={element.tabId}
            />
          ))}
        </ul>
        <ul className="appItem">
          {filteredApps.map(element => (
            <AppItem appItem={element} key={element.appId} />
          ))}
        </ul>
      </div>
    )
